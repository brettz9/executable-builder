...
exports.getHardPaths = function (emit) {
    ...
    return ['Desk', 'Strt', 'Progs', 'AppData', 'Pict', 'ProfD'].reduce(function (obj, name) {
...
// Todo: complete
// Todo: Option to preserve shortcut, SED, batch file (if converting to exe)
// Todo: UI to optionally pass in shortcut, hot key, windowStyle
exports.createBatchForShortcutCreation = function (data, emit) {
    ...
    var shortcutPath = data.shortcutPath,
        ...
        templateName = data.templateName || 'Firefox Shortcut',
        ...
        ':::oShellLink.Arguments = "-no-remote' + (profileName ? ' -P """' + stripQuotes(profileName) + '""' : '') + '"\n' + // Quotes not allowed in profile names anyways and double quotes could present an issue if already inside double quotes
        ':: -remote ""openurl(about:newtab)"""\n' + // Todo: reconcile
        // -webappdoc, -webappmode, -webappcustommode
        ':: & WScript.Arguments.Item(0)\n' + // Open With will pass the desktop file path to be opened here
        ':::oShellLink.Description = ' + batchQuote(templateName) + '\n' +
        ...
        ':::oShellLink.Save\n\n' +
        'findstr "^:::" "%~sf0">temp.vbs & cscript //nologo temp.vbs %1 & del temp.vbs\n';
    return batch;
};
