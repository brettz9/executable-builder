...
function buildSED (userSED) {
    ...
    function addUserSections (userSectionObject) {
        var userSectionName = getSectionName(userSectionObject),
            defaultSEDIdx = defaultSED.findIndex(findSection(userSectionName))
        if (defaultSEDIdx !== -1) {
            for (sub in userSectionObject) {
                defaultSED[defaultSEDIdx][userSectionName][sub] = userSectionObject[userSectionName][sub];
            }
        }
        else {
